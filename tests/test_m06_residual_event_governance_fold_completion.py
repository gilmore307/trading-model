...
from models.model_06_residual_event_governance.fold_completion import (
...
class M06ResidualEventGovernanceFoldCompletionTests(unittest.TestCase):
    def test_completion_keeps_calibrated_and_keyword_families_separate(self) -> None:
        with tempfile.TemporaryDirectory() as raw_tmp:
            ...
            completion = build_m06_residual_event_governance_fold_completion(
        ...
        self.assertFalse(completion.summary["m06_residual_event_governance_production_evidence_complete"])
    ...
    def test_writer_emits_summary_and_gate_matrix(self) -> None:
        with tempfile.TemporaryDirectory() as raw_tmp:
            ...
            out = tmp / "out"
            write_m06_residual_event_governance_fold_completion_artifacts(completion, out)

            self.assertTrue((out / "m06_residual_event_governance_fold_completion.json").exists())
            self.assertTrue((out / "m06_residual_event_governance_fold_completion_summary.json").exists())
            self.assertTrue((out / "m06_residual_event_governance_family_gate_matrix.csv").exists())
