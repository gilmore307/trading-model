...
from src.runners.monthly_review import run_monthly_review


def test_run_monthly_review_exports_artifacts_from_history(tmp_path: Path):
    ...
    history.write_text(
        json.dumps({
            'summary': {
                ...
                'account_metrics': {
                    'trend': {'pnl_usdt': 8.0, 'equity_end_usdt': 1008.0, 'fee_usdt': 0.2},
                    'router_composite': {'pnl_usdt': 5.0, 'fee_usdt': 0.1},
                    'flat_compare': {'pnl_usdt': 1.5},
                },
            },
            'compare_snapshot': {
    ...
    assert payload['meta']['label'] == 'monthly:2026-02-01->2026-03-01'
