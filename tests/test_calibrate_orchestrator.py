from src.review.calibrate_orchestrator import equity_report
...
def test_equity_report_flags_accounts_below_threshold(monkeypatch):
    monkeypatch.setattr('src.review.calibrate_orchestrator.OkxClientRegistry', FakeRegistry)
...
def test_equity_report_marks_all_ok_when_each_account_meets_threshold(monkeypatch):
    ...
    monkeypatch.setattr('src.review.calibrate_orchestrator.OkxClientRegistry', RegistryAllOk)
    report = equity_report(FakeSettings())
