...
class Layer10FoldCompletionTests(unittest.TestCase):
    ...
    def test_writer_emits_summary_and_gate_matrix(self) -> None:
        with tempfile.TemporaryDirectory() as raw_tmp:
            ...
            out = tmp / "out"
            write_layer_10_fold_completion_artifacts(completion, out)

            self.assertTrue((out / "layer_10_fold_completion.json").exists())
            self.assertTrue((out / "layer_10_fold_completion_summary.json").exists())
            self.assertTrue((out / "layer_10_family_gate_matrix.csv").exists())
