...
class SectorContextModelTests(unittest.TestCase):
    ...
    def test_generates_primary_rows_with_narrow_contract(self) -> None:
        ...
        self.assertIn(xlk["2_sector_handoff_bias"], {"long_bias", "short_bias", "neutral", "mixed"})
        self.assertIn(xlk["2_eligibility_state"], generator.ELIGIBILITY_STATES)
        self.assertIsNotNone(xlk["2_sector_relative_direction_score"])
        self.assertIsNotNone(xlk["2_sector_tradability_score"])
        self.assertNotIn("2_selection_readiness_score", xlk)
