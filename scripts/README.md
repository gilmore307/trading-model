...
- `generate_model_01_market_regime.py` reads `trading_data.feature_01_market_regime` and upserts the continuous state vector into `trading_model.model_01_market_regime`; by default it also writes per-ETF affinity rows into `trading_model.model_01_market_regime_etf_affinity`.
...
- `review_market_regime_promotion.py` builds an evaluation-backed promotion candidate and asks an OpenClaw agent to review whether it can be promoted. It emits review/decision rows only; it does not write promotion decisions or change a production pointer.
