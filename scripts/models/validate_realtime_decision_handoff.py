...
def main() -> int:
    ...
    parser.add_argument("payload_json", help="Path to execution_model_decision_input_snapshot or model_realtime_decision_route_plan JSON.")
    args = parser.parse_args()

    payload = json.loads(Path(args.payload_json).read_text(encoding="utf-8"))
    contract_type = payload.get("contract_type")
    if contract_type == "execution_model_decision_input_snapshot":
        result = validate_execution_model_decision_input_snapshot(payload)
    elif contract_type == "model_realtime_decision_route_plan":
