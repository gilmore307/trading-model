#!/usr/bin/env python3
"""Generate BackgroundContextModel rows from local JSON/JSONL rows."""
...
from models.model_01_background_context import MODEL_SURFACE, MODEL_VERSION

COLUMN_IDENTIFIER_RE = re.compile(r"^[A-Za-z0-9_]+$")
JSON_COLUMNS = {"background_context_state"}
TEXT_1_COLUMNS: set[str] = set()
...
def _column_type(column: str) -> str:
    ...
    if column.startswith("1_"):
        return "TEXT" if column in TEXT_1_COLUMNS else "DOUBLE PRECISION"
    return "TEXT"
...
def main(argv: list[str] | None = None) -> int:
    ...
    rows = generate_layer("models.model_01_background_context", input_rows, model_version=args.model_version)
