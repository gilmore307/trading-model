#!/usr/bin/env python3
"""Build PositionProjectionModel labels and evaluation summary from local or database rows."""
...
from models.model_06_position_projection import MODEL_ID, MODEL_SURFACE
...
def _fetch_model_rows(cursor: Any, *, schema: str, table: str, source_start: str | None, source_end: str | None) -> list[dict[str, Any]]:
    ...
    cursor.execute(f"SELECT * FROM {_qualified(schema, table)}{where_sql} ORDER BY available_time::timestamptz ASC, position_projection_vector_ref ASC", params)
...
def _evaluate(model_rows: list[dict[str, Any]], *, evidence_source: str, output_json: Path | None) -> None:
    ...
    payload = evaluate_layer(
        module_name="models.model_06_position_projection",
        label_builder_name="build_position_projection_labels",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--model-table", default="model_06_position_projection")
    ...
    if args.model_jsonl:
        ...
        model_rows = generate_layer("models.model_06_position_projection", input_rows)
