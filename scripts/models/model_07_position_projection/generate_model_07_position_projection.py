...
from models.model_07_position_projection import MODEL_ID, MODEL_SURFACE, MODEL_VERSION, generate_rows

DEFAULT_DB_URL_FILE = database_url_file()
IDENTIFIER_RE = re.compile(r"^[A-Za-z_][A-Za-z0-9_]*$")
COLUMN_IDENTIFIER_RE = re.compile(r"^[A-Za-z0-9_]+$")
JSON_COLUMNS = {"position_projection_vector", "position_projection_diagnostics", "7_horizon_resolution_reason_codes"}
PRIMARY_KEY = ("position_projection_vector_ref",)
EXPLAINABILITY_COLUMNS = {"position_projection_vector"}
DIAGNOSTICS_COLUMNS = {"position_projection_diagnostics", "7_horizon_resolution_reason_codes"}
...
def _column_type(column: str) -> str:
    ...
    if column.startswith("7_"):
        return "TEXT" if column in {"7_dominant_projection_horizon", "7_horizon_conflict_state"} else "DOUBLE PRECISION"
    return "TEXT"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--target-table", default="model_07_position_projection")
    ...
    rows = generate_layer("models.model_07_position_projection", input_rows, model_version=args.model_version)
