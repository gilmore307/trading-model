#!/usr/bin/env python3
"""Build local OptionExpressionModel labels and evaluation summary.

This entrypoint is intentionally local/fixture-safe. It checks inference rows for
label leakage where the model exposes a leakage checker, joins supplied outcome
...
from models.model_05_option_expression import MODEL_ID, MODEL_SURFACE
...
def _fetch_model_rows(cursor: Any, *, schema: str, table: str, source_start: str | None, source_end: str | None) -> list[dict[str, Any]]:
    ...
    cursor.execute(
        f"SELECT * FROM {_qualified(schema, table)}{where_sql} ORDER BY available_time::timestamptz ASC, option_expression_plan_ref ASC",
        params,
    )
...
def _evaluate(model_rows: list[dict[str, Any]], *, evidence_source: str, output_json: Path | None) -> None:
    ...
    payload = evaluate_layer(
        module_name="models.model_05_option_expression",
        label_builder_name="build_option_expression_labels",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--model-table", default="model_05_option_expression")
    ...
    if args.model_jsonl:
        ...
        model_rows = generate_layer("models.model_05_option_expression", input_rows)
