...
def _fetch_model_04_rows(cursor: Any, *, source_start: str | None, source_end: str | None) -> list[dict[str, Any]]:
    ...
    if support_exists:
        cursor.execute(
            ...
            LEFT JOIN {_qualified('trading_data', 'model_02_target_state_data_acquisition')} AS s
        ...
        rows.append(
            {
                ...
                "underlying_quote_snapshot_ref": None if not underlying else f"model_02_target_state_data_acquisition:{row.get('target_candidate_id')}:{_time_key(available_time)}",
