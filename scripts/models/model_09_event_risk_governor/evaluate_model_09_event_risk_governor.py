...
from models.model_09_event_risk_governor import MODEL_ID, MODEL_SURFACE
...
def _fetch_model_rows(cursor: Any, *, schema: str, table: str, source_start: str | None, source_end: str | None) -> list[dict[str, Any]]:
    ...
    cursor.execute(f"SELECT * FROM {_qualified(schema, table)}{where_sql} ORDER BY available_time ASC, event_context_vector_ref ASC", params)
...
def _evaluate(model_rows: list[dict[str, Any]], *, evidence_source: str, output_json: Path | None) -> None:
    ...
    payload = evaluate_layer(
        module_name="models.model_09_event_risk_governor",
        label_builder_name="build_event_risk_governor_labels",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--model-table", default="model_09_event_risk_governor")
    ...
    if args.model_jsonl:
        ...
        model_rows = generate_layer("models.model_09_event_risk_governor", input_rows)
