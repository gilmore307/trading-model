...
from models.model_09_event_risk_governor import MODEL_ID, MODEL_SURFACE, MODEL_VERSION, generate_rows
...
def _column_type(column: str) -> str:
    if column in JSON_COLUMNS:
        return "JSONB"
    if column.startswith("9_") and "legacy" not in column:
    ...
    *,
    ...
    with psycopg.connect(database_url, row_factory=dict_row) as conn:
        with conn.cursor() as cursor:
            source_rows = _fetch_rows(cursor, schema="trading_data", table="source_09_event_risk_governor", source_start=source_start, source_end=source_end, order_by="available_time ASC, event_id ASC")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--target-table", default="model_09_event_risk_governor")
    ...
    rows = generate_layer("models.model_09_event_risk_governor", input_rows, model_version=args.model_version)
