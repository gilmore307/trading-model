#!/usr/bin/env python3
"""Build EventFailureRiskModel labels and evaluation summary from local or database rows."""
...
from model_governance.local_layer_scripts import FIXTURE_INPUT_ROWS, evaluate_layer, fixture_outcome_rows, generate_layer, read_rows, write_payload
from models.model_04_event_failure_risk import MODEL_ID, MODEL_SURFACE
...
def _fetch_model_rows(cursor: Any, *, schema: str, table: str, source_start: str | None, source_end: str | None) -> list[dict[str, Any]]:
    ...
    cursor.execute(f"SELECT * FROM {_qualified(schema, table)}{where_sql} ORDER BY available_time::timestamptz ASC, event_failure_risk_vector_ref ASC", params)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--model-table", default="model_04_event_failure_risk")
    ...
    if args.from_database:
        ...
        model_rows = generate_layer("models.model_04_event_failure_risk", input_rows)
    ...
    payload = evaluate_layer(
        module_name="models.model_04_event_failure_risk",
        label_builder_name="build_event_failure_risk_labels",
