...
The current package consumes the Layer 7 underlying thesis plus
...
def generate_rows(input_rows: Iterable[Mapping[str, Any]], *, model_version: str = MODEL_VERSION) -> list[dict[str, Any]]:
    ...
    if not rows:
        raise ValueError("at least one Layer 9 input row is required")
...
def _model_row(row: Mapping[str, Any], *, model_version: str) -> dict[str, Any]:
    ...
    handoff = (
        _payload(underlying_plan, "handoff_to_layer_9")
        or _payload(underlying_plan, "handoff_to_layer_8")
        or _payload(row, "layer_9_underlying_handoff")
    ...
    for horizon, payload in horizon_payloads.items():
        ...
        score_payload.update(
            {
                f"9_option_expression_eligibility_score_{suffix}": payload["expression_eligibility_score"],
                f"9_option_expression_direction_score_{suffix}": payload["expression_direction_score"],
                f"9_option_contract_fit_score_{suffix}": payload["contract_fit_score"],
                f"9_option_liquidity_fit_score_{suffix}": payload["liquidity_fit_score"],
                f"9_option_iv_fit_score_{suffix}": payload["iv_fit_score"],
                f"9_option_greek_fit_score_{suffix}": payload["greek_fit_score"],
                f"9_option_reward_risk_score_{suffix}": payload["reward_risk_score"],
                f"9_option_theta_risk_score_{suffix}": payload["theta_risk_score"],
                f"9_option_fill_quality_score_{suffix}": payload["fill_quality_score"],
    ...
    resolved_payload = {
        ...
        "9_resolved_expression_confidence_score": dominant["expression_confidence_score"],
        "9_resolved_no_option_reason_codes": no_option_reason_codes,
        "9_resolved_reason_codes": reason_codes,
    }
    output = {
...
def _normalize_input_row(row: Mapping[str, Any]) -> dict[str, Any]:
    output = dict(row)
    for key in ("underlying_action_plan", "layer_9_underlying_handoff", "layer_8_underlying_handoff", "market_context_state", "event_context_vector", "option_expression_policy"):
