...
This module starts from `base_stack_layers_01_08` evaluation labels instead of raw price moves.
...
from models.model_08_event_risk_governor.price_anomaly_event_discovery import _event_family_dates, _nearby, _parse_dt
...
DEFAULT_SOURCE_ROOT = trading_data_root() / "storage/monthly_backfill_v1"
...
class ResidualAnomalyEventDiscovery:
    ...
    def summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "discovery_note": "Starts from base_stack_layers_01_08 evaluation residuals, then searches nearby PIT event families. Promotion requires an evidence packet plus agent review.",
        }
...
def _load_plan_reason_codes(runtime_root: Path, evaluation_month: str) -> dict[str, tuple[str, ...]]:
    ...
    with path.open(encoding="utf-8") as handle:
        for line in handle:
            ...
            if not ref:
                continue
            reasons = _coerce_reason_codes(row.get("9_resolved_reason_codes"))
            if not reasons:
...
This artifact starts from `base_stack_layers_01_08` evaluation residuals, not raw price anomalies. It then searches nearby point-in-time event families for explanations, observation-pool candidates, and strategy-promotion review packets. Strategy promotion remains blocked until an emitted packet receives agent review.
