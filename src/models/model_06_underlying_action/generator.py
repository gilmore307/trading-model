...
def _model_row(row: Mapping[str, Any], *, model_version: str) -> dict[str, Any]:
    ...
    for horizon, payload in horizon_payloads.items():
        ...
        score_payload.update(
            {
                f"6_underlying_trade_eligibility_score_{suffix}": payload["trade_eligibility_score"],
                f"6_underlying_action_direction_score_{suffix}": payload["action_direction_score"],
                f"6_underlying_trade_intensity_score_{suffix}": payload["trade_intensity_score"],
                f"6_underlying_entry_quality_score_{suffix}": payload["entry_quality_score"],
                f"6_underlying_expected_return_score_{suffix}": payload["expected_return_score"],
                f"6_underlying_adverse_risk_score_{suffix}": payload["adverse_risk_score"],
                f"6_underlying_reward_risk_score_{suffix}": payload["reward_risk_score"],
                f"6_underlying_liquidity_fit_score_{suffix}": payload["liquidity_fit_score"],
                f"6_underlying_holding_time_fit_score_{suffix}": payload["holding_time_fit_score"],
                f"6_underlying_action_confidence_score_{suffix}": payload["action_confidence_score"],
            }
        )

    resolved_payload = {
        "6_resolved_underlying_action_type": action["planned_underlying_action_type"],
        "6_resolved_action_side": action["action_side"],
        "6_resolved_dominant_horizon": dominant_horizon,
        "6_resolved_trade_eligibility_score": dominant["trade_eligibility_score"],
        "6_resolved_trade_intensity_score": dominant["trade_intensity_score"],
        "6_resolved_entry_quality_score": dominant["entry_quality_score"],
        "6_resolved_action_confidence_score": dominant["action_confidence_score"],
        "6_resolved_reason_codes": reason_codes,
    }
...
def _resolved_target_exposure(projection: Mapping[str, Any]) -> float:
    resolved = _first_float(projection, "5_resolved_target_exposure_score", "target_underlying_exposure_score", "target_exposure_score")
    if resolved is not None:
        return _clip_signed(resolved)
    values = [_safe_float(projection.get(f"5_target_exposure_score_{_suffix(horizon)}")) for horizon in HORIZONS]
...
def _gate_state(
    ...
    risk_fit = _first_float(risk, "risk_budget_fit_score", "5_risk_budget_fit_score", "risk_fit_score")
    return {
...
def _horizon_payload(
    ...
    suffix = _suffix(horizon)
    alpha_confidence = _score(alpha, f"4_alpha_confidence_score_{suffix}", "4_alpha_confidence_score", default=0.5)
    expected_return = _first_float(alpha, f"4_expected_return_score_{suffix}", "4_expected_return_score")
    if expected_return is None:
        expected_return = _first_float(projection, f"5_expected_position_utility_score_{suffix}", "5_expected_position_utility_score")
    if expected_return is None:
        expected_return = 0.0
    projection_confidence = _score(projection, f"5_projection_confidence_score_{suffix}", "5_projection_confidence_score", default=0.5)
    risk_fit = _score(projection, f"5_risk_budget_fit_score_{suffix}", "5_risk_budget_fit_score", default=gate_state["risk_budget_fit_score"])
    cost_to_adjust = _score(projection, f"5_cost_to_adjust_position_score_{suffix}", "5_cost_to_adjust_position_score", default=0.2)
    stability = _score(projection, f"5_position_state_stability_score_{suffix}", "5_position_state_stability_score", default=0.6)
    path_quality = _score(alpha, f"4_path_quality_score_{suffix}", "4_path_quality_score", default=0.6)
    reversal_risk = _score(alpha, f"4_reversal_risk_score_{suffix}", "4_reversal_risk_score", default=0.25)
    drawdown_risk = _score(alpha, f"4_drawdown_risk_score_{suffix}", "4_drawdown_risk_score", default=0.25)
...
def _dominant_horizon(horizon_payloads: Mapping[str, Mapping[str, Any]], projection: Mapping[str, Any]) -> str:
    explicit = str(projection.get("5_dominant_projection_horizon") or "").strip()
