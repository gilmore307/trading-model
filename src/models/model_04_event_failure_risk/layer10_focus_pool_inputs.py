...
def build_layer4_focus_pool_input_rows(
    *,
    ...
    model_version: str = "layer10_focus_pool_contract",
) -> list[dict[str, Any]]:
    accepted = {
        ...
        output.append(
            {
                ...
                "target_candidate_id": f"layer10_replay_{decision_id}",
                "layer_10_event_contract_ref": row.get("event_context_vector_ref"),
                "layer_10_event_contract": {
                    ...
                    "10_event_presence_score_1D": row.get(f"10_event_presence_score_{suffix}"),
                    "10_event_timing_proximity_score_1D": row.get(f"10_event_timing_proximity_score_{suffix}"),
                    "10_event_intensity_score_1D": row.get(f"10_event_intensity_score_{suffix}"),
                    "10_event_gap_risk_score_1D": row.get(f"10_event_gap_risk_score_{suffix}"),
                    "10_event_reversal_risk_score_1D": row.get(f"10_event_reversal_risk_score_{suffix}"),
                    "10_event_underlying_impact_score_1D": row.get(f"10_event_underlying_impact_score_{suffix}"),
                    "10_event_option_impact_score_1D": row.get(f"10_event_option_impact_score_{suffix}"),
                },
            }
        )
    return output
