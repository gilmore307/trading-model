...
- `model_sequence.py` — serializable `M01` through `M05` display/order metadata for the current model stack.
...
- `return_distribution_surface/` — shared tradable-time conditional return distribution surface utilities that M01 through M05 will progressively adopt.
