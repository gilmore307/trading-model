...
def generate_rows(input_rows: Iterable[Mapping[str, Any]], *, model_version: str = MODEL_VERSION) -> list[dict[str, Any]]:
    ...
    if not rows:
        raise ValueError("at least one Layer 9 event-risk decision/context row is required")
...
def _model_row(row: Mapping[str, Any], *, model_version: str) -> dict[str, Any]:
    ...
    for horizon in HORIZONS:
        ...
        payload.update({f"9_event_{key}_score_{suffix}": scores[key] for key in (
        ...
        payload[f"9_event_direction_bias_score_{suffix}"] = scores["direction_bias"]
        payload[f"9_event_context_alignment_score_{suffix}"] = scores["context_alignment"]
        for scope in SCOPE_KEYS:
            payload[f"9_event_{scope}_impact_score_{suffix}"] = scores[f"{scope}_impact"]
        payload[f"9_event_scope_confidence_score_{suffix}"] = scores["scope_confidence"]
        payload[f"9_event_scope_escalation_risk_score_{suffix}"] = scores["scope_escalation_risk"]
        payload[f"9_event_target_relevance_score_{suffix}"] = scores["target_relevance"]
        audit[f"9_event_dominant_impact_scope_{suffix}"] = scores["dominant_impact_scope"]
    ...
    output = {
        ...
        **payload,
        ...
        "event_risk_governor_diagnostics": {
            ...
            "underlying_thesis_context": _underlying_thesis_context(row),
        },
    }
...
def _visible_events(row: Mapping[str, Any], decision_available_time: datetime) -> list[Mapping[str, Any]]:
    event_rows = row.get("source_09_event_risk_governor") or row.get("event_rows") or row.get("events") or []
...
def _normalize_row(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    events = output.get("source_09_event_risk_governor") or output.get("event_rows") or output.get("events") or []
...
def _underlying_thesis_context(row: Mapping[str, Any]) -> dict[str, Any]:
