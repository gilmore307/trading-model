...
produce signed direction, or activate event-risk controls.
"""
...
def run_prior_guidance_exhibit_extraction(inputs: PriorGuidanceExhibitExtractionInputs) -> dict[str, Any]:
    ...
    report = {
        "schema": "earnings_guidance_prior_guidance_exhibit_extraction",
...
Conclusion: exhibit-level official filing text reduces the prior-guidance baseline gap, but it still does not establish guidance surprise, raise/cut, signed direction, alpha, model activation, or M03 event-state governance escalation.
""",
        encoding="utf-8",
    )
    return report
