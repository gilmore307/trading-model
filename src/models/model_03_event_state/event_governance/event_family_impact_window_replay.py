...
This module joins frozen replay decision rows to the M03 event-impact
impact-window artifact, then runs the EventState generator over the
...
from models.model_03_event_state import generate_rows
from models.model_03_event_state.event_governance.event_family_impact_window_backtest import DEFAULT_CANDIDATE_WINDOWS
from models.model_03_event_state.event_governance.event_family_batch_catalog import build_event_family_batch_catalog
from models.model_03_event_state.event_governance.event_family_empirical_coverage import FAMILY_KEYWORDS
from models.model_03_event_state.event_governance.event_observation_pool_policy import build_event_observation_pool_policy
...
def _generator_input(decision: Mapping[str, Any], events: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "event_observations": list(events),
    }
...
def _summarize(
    *,
    ...
    for decision, input_row in zip(decision_rows, input_rows):
        decision_events = input_row.get("event_observations") if isinstance(input_row.get("event_observations"), list) else []
        if decision_events:
            visible_rows += 1
...
def build_impact_window_replay_artifacts(
    *,
    ...
    for decision, input_row, model_row in zip(decision_rows, input_rows, model_rows):
        diagnostics = model_row.get("event_state_diagnostics") if isinstance(model_row.get("event_state_diagnostics"), Mapping) else {}
        overlay_rows.append(
            {
                ...
                "visible_event_families": [event.get("event_family_key") for event in input_row.get("event_observations", [])],
                "visible_event_window_policies": [event.get("window_policy") for event in input_row.get("event_observations", [])],
                "event_state_vector_ref": model_row.get("event_state_vector_ref"),
                "3_event_response_strength_score_1D": model_row.get("3_event_response_strength_score_1D"),
                "3_event_path_risk_score_1D": model_row.get("3_event_path_risk_score_1D"),
                "3_event_underlying_price_impact_score_1D": model_row.get("3_event_underlying_price_impact_score_1D"),
                "3_event_option_price_impact_score_1D": model_row.get("3_event_option_price_impact_score_1D"),
                "3_event_target_relevance_score_1D": model_row.get("3_event_target_relevance_score_1D"),
            }
        )

    input_path = output_dir / "model_03_event_state_replay_input_rows.jsonl"
    model_path = output_dir / "model_03_event_state_rows.jsonl"
    ...
    (output_dir / "README.md").write_text(
        ...
        "This artifact applies reviewed local M03 event-impact context to frozen replay decision rows. "
