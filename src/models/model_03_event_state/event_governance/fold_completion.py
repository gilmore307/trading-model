"""Fold-scoped M03 event-family completion evidence.

This module consolidates the normal M03 event-state governance family workflow gates for
...
CONTRACT_TYPE = "event_family_fold_completion"
SUMMARY_CONTRACT_TYPE = "event_family_fold_completion_summary"

DEFAULT_FOLD_ID = "fold_aapl_2016"
DEFAULT_REPLAY_RUN_ID = "model_group_replay_20260609T060059Z"
DEFAULT_OUTPUT_DIR = model_storage_root() / "event_family_fold_completion_20260610" / DEFAULT_FOLD_ID / DEFAULT_REPLAY_RUN_ID
...
class EventFamilyCompletionRow:
    family_key: str
    in_catalog: bool
...
@dataclass(frozen=True)
class EventFamilyFoldCompletion:
    contract_type: str
    generated_at_utc: str
    fold_id: str
    replay_run_id: str
    family_rows: tuple[EventFamilyCompletionRow, ...]
    ...
    def summary(self) -> dict[str, Any]:
        return {
            ...
            "event_family_effect_model_evidence_complete": all(
...
def _production_route_review_status(
    *,
    ...
    return "agent_review_deferred_incomplete_event_family_effect_model_workflow"
...
def _production_route_decision(
    *,
    ...
    if review_status != "agent_review_complete":
        return "defer_incomplete_event_family_effect_model_workflow"
...
def build_event_family_fold_completion(
    *,
    ...
    rows: list[EventFamilyCompletionRow] = []
    ...
    return EventFamilyFoldCompletion(
...
def write_event_family_fold_completion_artifacts(completion: EventFamilyFoldCompletion, output_dir: Path) -> None:
    output_dir.mkdir(parents=True, exist_ok=True)
    (output_dir / "event_family_fold_completion.json").write_text(
        json.dumps(completion.to_dict(), indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    (output_dir / "event_family_fold_completion_summary.json").write_text(
...
def write_completion(completion: EventFamilyFoldCompletion, *, output: TextIO) -> None:
