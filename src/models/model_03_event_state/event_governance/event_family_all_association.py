"""Local all-family event/price association measurement for M03 event-state governance.
...
This artifact emits an association row for every M03 event-state governance family. It separates measured association from no-local-event and blocked-precondition statuses. It uses local source/study artifacts only and performs no provider calls, training, activation, broker/account mutation, destructive SQL, or artifact deletion.
