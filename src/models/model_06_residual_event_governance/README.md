...
The current deterministic pilot consumes M01 background refs, M02 target refs,
M03 event-state refs, the M04 `unified_decision_vector_ref` /
`direct_underlying_intent`, an optional M05 `option_expression_plan_ref` /
`option_expression_plan`, and point-in-time event observations. It scores
underlying-price and option-price event-impact channels natively under M06,
