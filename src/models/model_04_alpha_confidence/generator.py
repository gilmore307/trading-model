...
def _model_row(row: Mapping[str, Any], *, model_version: str) -> dict[str, Any]:
    ...
    for horizon in HORIZONS:
        ...
        final_payload.update(
            {
                f"4_alpha_direction_score_{suffix}": calibrated["alpha_direction"],
                f"4_alpha_strength_score_{suffix}": calibrated["alpha_strength"],
                f"4_expected_return_score_{suffix}": calibrated["expected_return"],
                f"4_alpha_confidence_score_{suffix}": calibrated["alpha_confidence"],
                f"4_signal_reliability_score_{suffix}": calibrated["signal_reliability"],
                f"4_path_quality_score_{suffix}": calibrated["path_quality"],
                f"4_reversal_risk_score_{suffix}": calibrated["reversal_risk"],
                f"4_drawdown_risk_score_{suffix}": calibrated["drawdown_risk"],
                f"4_alpha_tradability_score_{suffix}": calibrated["alpha_tradability"],
            }
        )
        base_payload.update(
            {
                f"4_base_alpha_direction_score_{suffix}": base["direction"],
                f"4_base_alpha_strength_score_{suffix}": base["strength"],
                ...
                f"4_base_alpha_tradability_score_{suffix}": base["tradability"],
...
def _adjust_for_events(horizon: str, base: Mapping[str, float], event: Mapping[str, Any]) -> dict[str, Any]:
    suffix = _suffix(horizon)
    presence = _score(event, f"8_event_presence_score_{suffix}", default=0.0)
    intensity = _score(event, f"8_event_intensity_score_{suffix}", default=0.0)
    relevance = _score(event, f"8_event_target_relevance_score_{suffix}", default=0.0)
    event_quality = _score(event, f"8_event_context_quality_score_{suffix}", default=0.65)
    event_direction = _signed(event, f"8_event_direction_bias_score_{suffix}")
    alignment = _signed(event, f"8_event_context_alignment_score_{suffix}")
    uncertainty = _score(event, f"8_event_uncertainty_score_{suffix}", default=0.0)
    reversal = _score(event, f"8_event_reversal_risk_score_{suffix}", default=0.0)
    gap = _score(event, f"8_event_gap_risk_score_{suffix}", default=0.0)
    liquidity = _score(event, f"8_event_liquidity_disruption_score_{suffix}", default=0.0)
...
def _calibrate(
    ...
    layer_quality = min(
        ...
        _score(event, f"8_event_context_quality_score_{suffix}", default=0.75),
        data_quality,
    )
