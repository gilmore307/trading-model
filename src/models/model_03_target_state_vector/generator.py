...
Consumes point-in-time ``model_03_target_state_vector_feature_generation`` rows and emits the
...
def generate_rows(feature_rows: Iterable[Mapping[str, Any]], *, model_version: str = MODEL_VERSION) -> list[dict[str, Any]]:
    ...
    if not rows:
        raise ValueError("at least one model_03_target_state_vector_feature_generation row is required")
