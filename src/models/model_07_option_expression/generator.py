...
The current package consumes the conceptual Layer 6 underlying thesis plus
...
def _model_row(row: Mapping[str, Any], *, model_version: str) -> dict[str, Any]:
    ...
    for horizon, payload in horizon_payloads.items():
        ...
        score_payload.update(
            {
                f"7_option_expression_eligibility_score_{suffix}": payload["expression_eligibility_score"],
                f"7_option_expression_direction_score_{suffix}": payload["expression_direction_score"],
                f"7_option_contract_fit_score_{suffix}": payload["contract_fit_score"],
                f"7_option_liquidity_fit_score_{suffix}": payload["liquidity_fit_score"],
                f"7_option_iv_fit_score_{suffix}": payload["iv_fit_score"],
                f"7_option_greek_fit_score_{suffix}": payload["greek_fit_score"],
                f"7_option_reward_risk_score_{suffix}": payload["reward_risk_score"],
                f"7_option_theta_risk_score_{suffix}": payload["theta_risk_score"],
                f"7_option_fill_quality_score_{suffix}": payload["fill_quality_score"],
                f"7_option_expression_confidence_score_{suffix}": payload["expression_confidence_score"],
            }
        )
    ...
    resolved_payload = {
        "7_resolved_expression_type": expression_type,
        "7_resolved_option_right": option_right,
        "7_resolved_dominant_horizon": dominant_horizon,
        "7_resolved_selected_contract_ref": None if selected is None else selected["contract_ref"],
        "7_resolved_contract_fit_score": dominant["contract_fit_score"],
        "7_resolved_expression_confidence_score": dominant["expression_confidence_score"],
        "7_resolved_no_option_reason_codes": no_option_reason_codes,
...
def _underlying_direction(underlying_plan: Mapping[str, Any], handoff: Mapping[str, Any]) -> str:
    ...
    if direction in {"bullish", "bearish", "neutral"}:
        return direction
    side = str(underlying_plan.get("action_side") or underlying_plan.get("6_resolved_action_side") or "").lower()
    if side == "long":
        return "bullish"
...
def _horizon_payload(
    ...
    event_uncertainty = _score(event, f"8_event_uncertainty_score_{_suffix(horizon)}", default=0.15)
...
def _dominant_horizon(horizon_payloads: Mapping[str, Mapping[str, Any]], underlying_plan: Mapping[str, Any]) -> str:
    resolved = str(underlying_plan.get("dominant_horizon") or underlying_plan.get("6_resolved_dominant_horizon") or "")
...
def _target_delta(handoff: Mapping[str, Any], event: Mapping[str, Any]) -> float:
    path_quality = _score(handoff, "path_quality_score", default=0.5)
    event_gap = _score(event, "8_event_gap_risk_score_390min", default=0.0)
