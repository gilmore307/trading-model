...
The current generator consumes `direct_underlying_intent` / `unified_decision_vector_ref` from M04 plus point-in-time option-chain candidates. It emits `option_expression_plan`, `expression_vector`, and `5_*` score/resolution fields. Retired `underlying_action_plan` input is accepted only as migration fallback and is not emitted by the current M05 contract.
