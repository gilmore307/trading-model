...
def _model_row(row: Mapping[str, Any], *, model_version: str) -> dict[str, Any]:
    ...
    handoff = (
        ...
        or _payload(underlying_intent, "handoff_to_layer_9")
        or _payload(row, "model_05_underlying_handoff")
        or _payload(row, "layer_8_underlying_handoff")
    )
    ...
    for horizon, payload in horizon_payloads.items():
        ...
        score_payload.update(
            {
                f"5_option_expression_eligibility_score_{suffix}": payload["expression_eligibility_score"],
                f"5_option_expression_direction_score_{suffix}": payload["expression_direction_score"],
                f"5_option_contract_fit_score_{suffix}": payload["contract_fit_score"],
                f"5_option_liquidity_fit_score_{suffix}": payload["liquidity_fit_score"],
                f"5_option_iv_fit_score_{suffix}": payload["iv_fit_score"],
                f"5_option_greek_fit_score_{suffix}": payload["greek_fit_score"],
                f"5_option_reward_risk_score_{suffix}": payload["reward_risk_score"],
                f"5_option_theta_risk_score_{suffix}": payload["theta_risk_score"],
                f"5_option_fill_quality_score_{suffix}": payload["fill_quality_score"],
    ...
    resolved_payload = {
        ...
        "5_resolved_option_right": option_right,
        "5_resolved_option_surface_status": option_surface_status,
        "5_resolved_dominant_horizon": dominant_horizon,
        "5_resolved_selected_contract_ref": None if selected is None else selected["contract_ref"],
        "5_resolved_contract_fit_score": dominant["contract_fit_score"],
        "5_resolved_expression_confidence_score": dominant["expression_confidence_score"],
        "5_resolved_no_option_reason_codes": no_option_reason_codes,
        "5_resolved_reason_codes": reason_codes,
    }
    output = {
...
def _underlying_direction(underlying_intent: Mapping[str, Any], handoff: Mapping[str, Any]) -> str:
    direction = str(handoff.get("underlying_path_direction") or "").lower()
    if direction in {"bullish", "bearish", "neutral"}:
        return direction
    side = str(underlying_intent.get("action_side") or underlying_intent.get("4_resolved_action_side") or underlying_intent.get("8_resolved_action_side") or "").lower()
    if side == "long":
        return "bullish"
...
def _horizon_payload(
    ...
    if expression_type == "underlying_only_expression":
        ...
        event_uncertainty = _score(event, f"10_event_uncertainty_score_{_suffix(horizon)}", default=0.15)
...
def _dominant_horizon(horizon_payloads: Mapping[str, Mapping[str, Any]], underlying_intent: Mapping[str, Any]) -> str:
    resolved = str(
        underlying_intent.get("dominant_horizon")
        or underlying_intent.get("4_resolved_decision_horizon")
        or underlying_intent.get("8_resolved_dominant_horizon")
        or ""
    )
