...
def _model_row(row: Mapping[str, Any], *, model_version: str) -> dict[str, Any]:
    ...
    for horizon in HORIZONS:
        ...
        payload.update(
            {
                f"7_target_position_bias_score_{suffix}": detail["target_position_bias"],
                f"7_target_exposure_score_{suffix}": detail["target_exposure"],
                f"7_current_position_alignment_score_{suffix}": detail["current_position_alignment"],
                f"7_position_gap_score_{suffix}": detail["position_gap"],
                f"7_position_gap_magnitude_score_{suffix}": detail["position_gap_magnitude"],
                f"7_expected_position_utility_score_{suffix}": detail["expected_position_utility"],
                f"7_cost_to_adjust_position_score_{suffix}": detail["cost_to_adjust_position"],
                f"7_risk_budget_fit_score_{suffix}": detail["risk_budget_fit"],
                f"7_position_state_stability_score_{suffix}": detail["position_state_stability"],
                f"7_projection_confidence_score_{suffix}": detail["projection_confidence"],
            }
        )
...
def _resolve_horizon(details: Mapping[str, Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "7_dominant_projection_horizon": dominant_horizon,
        "7_horizon_conflict_state": conflict,
        "7_resolved_target_exposure_score": round(float(dominant["target_exposure"]), 6),
        "7_resolved_position_gap_score": round(float(dominant["position_gap"]), 6),
        "7_projection_resolution_confidence_score": round(confidence, 6),
        "7_horizon_resolution_reason_codes": reason_codes,
    }
