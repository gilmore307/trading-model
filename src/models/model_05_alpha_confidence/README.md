...
Owns local conversion from reviewed Layer 1/2/3 state plus Layer 4 `event_failure_risk_vector` into the final adjusted `alpha_confidence_vector`. Generation requires trained after-cost score artifacts from `training.py`; missing artifacts block generation. It implements:
...
- direct normalized after-cost alpha score inference;
