...
def _model_row(row: Mapping[str, Any], *, model_version: str, after_cost_alpha_model: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    event = _payload(row, "event_failure_risk_vector")
...
def _adjust_for_events(horizon: str, base: Mapping[str, float], event: Mapping[str, Any]) -> dict[str, Any]:
    suffix = _suffix(horizon)
    applicability = _score(event, f"4_event_applicability_confidence_score_{suffix}", default=0.0)
    failure_risk = _score(event, f"4_event_strategy_failure_risk_score_{suffix}", default=0.0)
    entry_block = _score(event, f"4_event_entry_block_pressure_score_{suffix}", default=0.0)
    evidence_quality = _score(event, f"4_event_evidence_quality_score_{suffix}", default=0.65)
    disable = _score(event, f"4_event_strategy_disable_pressure_score_{suffix}", default=0.0)
    path_gap = _score(event, f"4_event_path_risk_amplifier_score_{suffix}", default=0.0)
    session_gap = _score(event, f"4_event_session_gap_risk_score_{suffix}", default=0.0)
    gap = max(path_gap, session_gap)
    exposure_cap = _score(event, f"4_event_exposure_cap_pressure_score_{suffix}", default=0.0)
    event_weight = _clip01(applicability * max(failure_risk, entry_block, exposure_cap, disable, gap) * evidence_quality)
    override = bool(event_weight >= 0.55 and max(entry_block, disable) >= 0.65 and evidence_quality >= 0.70)
    direction_adjustment = 0.0
    strength_adjustment = -0.35 * event_weight
    risk_adjustment = _clip01((failure_risk * 0.35 + entry_block * 0.25 + gap * 0.25 + exposure_cap * 0.15) * max(applicability, event_weight))
    adjusted_direction = _clip_signed(float(base["direction"]))
    adjusted_strength = _clip01(float(base["strength"]) + strength_adjustment - 0.25 * risk_adjustment)
    path_quality = _clip01(float(base["path_quality"]) - risk_adjustment * 0.35 - 0.15 * max(entry_block, disable))
    return {
        ...
        "path_quality": round(path_quality, 6),
        "reversal_risk": round(_clip01(float(base["reversal_risk"]) + entry_block * event_weight * 0.40 + risk_adjustment * 0.20), 6),
        "drawdown_risk": round(_clip01(float(base["drawdown_risk"]) + gap * event_weight * 0.35 + exposure_cap * event_weight * 0.20), 6),
        "tradability": round(_clip01(float(base["tradability"]) - event_weight * 0.20 - risk_adjustment * 0.35), 6),
...
def _calibrate(
    ...
    layer_quality = min(
        ...
        _score(event, f"4_event_evidence_quality_score_{suffix}", default=0.75),
        data_quality,
    )
