...
- `event_family_impact_window_real_inputs.py` builds the 2026-06-10 reviewed local input route from Trading Economics calendar files plus SQL-retained Alpaca/GDELT news and point-in-time ETF bars; it emits evidence artifacts only and does not train, activate, trade, write SQL, or delete artifacts.
- `event_family_impact_window_replay.py` applies reviewed impact-window parameters and SQL-retained candidate-family observations to frozen replay decision rows, separating calibrated event windows from uncalibrated same-day observation diagnostics without changing replay, SQL, model activation, broker, or account state.
