...
def generate_rows(input_rows: Iterable[Mapping[str, Any]], *, model_version: str = MODEL_VERSION) -> list[dict[str, Any]]:
    ...
    if not rows:
        raise ValueError("at least one Layer 10 event-risk decision/context row is required")
...
def _model_row(row: Mapping[str, Any], *, model_version: str) -> dict[str, Any]:
    ...
    for horizon in HORIZONS:
        ...
        payload.update({f"10_event_{key}_score_{suffix}": scores[key] for key in (
        ...
        payload[f"10_event_direction_bias_score_{suffix}"] = scores["direction_bias"]
        ...
        audit[f"10_event_dominant_impact_scope_{suffix}"] = scores["dominant_impact_scope"]
...
def _visible_events(row: Mapping[str, Any], decision_available_time: datetime) -> list[Mapping[str, Any]]:
    event_rows = row.get("m06_residual_event_governance_data_acquisition") or row.get("event_rows") or row.get("events") or []
...
def _normalize_row(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    events = output.get("m06_residual_event_governance_data_acquisition") or output.get("event_rows") or output.get("events") or []
