...
def _model_row(row: Mapping[str, Any], *, model_version: str) -> dict[str, Any]:
    ...
    for horizon in HORIZONS:
        ...
        payload.update(
            {
                f"5_target_position_bias_score_{suffix}": detail["target_position_bias"],
                f"5_target_exposure_score_{suffix}": detail["target_exposure"],
                f"5_current_position_alignment_score_{suffix}": detail["current_position_alignment"],
                f"5_position_gap_score_{suffix}": detail["position_gap"],
                f"5_position_gap_magnitude_score_{suffix}": detail["position_gap_magnitude"],
                f"5_expected_position_utility_score_{suffix}": detail["expected_position_utility"],
                f"5_cost_to_adjust_position_score_{suffix}": detail["cost_to_adjust_position"],
                f"5_risk_budget_fit_score_{suffix}": detail["risk_budget_fit"],
                f"5_position_state_stability_score_{suffix}": detail["position_state_stability"],
                f"5_projection_confidence_score_{suffix}": detail["projection_confidence"],
            }
        )
...
def _horizon_projection(
    ...
    suffix = _suffix(horizon)
    direction = _signed(alpha, f"4_alpha_direction_score_{suffix}")
    strength = _score(alpha, f"4_alpha_strength_score_{suffix}", default=0.0)
    expected_return = _signed(alpha, f"4_expected_return_score_{suffix}")
    confidence = _score(alpha, f"4_alpha_confidence_score_{suffix}", default=0.0)
    reliability = _score(alpha, f"4_signal_reliability_score_{suffix}", default=0.0)
    path_quality = _score(alpha, f"4_path_quality_score_{suffix}", default=0.5)
    reversal = _score(alpha, f"4_reversal_risk_score_{suffix}", default=0.5)
    drawdown = _score(alpha, f"4_drawdown_risk_score_{suffix}", default=0.5)
    alpha_tradability = _score(alpha, f"4_alpha_tradability_score_{suffix}", default=0.0)
...
def _resolve_horizon(details: Mapping[str, Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "5_dominant_projection_horizon": dominant_horizon,
        "5_horizon_conflict_state": conflict,
        "5_resolved_target_exposure_score": round(float(dominant["target_exposure"]), 6),
        "5_resolved_position_gap_score": round(float(dominant["position_gap"]), 6),
        "5_projection_resolution_confidence_score": round(confidence, 6),
        "5_horizon_resolution_reason_codes": reason_codes,
    }
