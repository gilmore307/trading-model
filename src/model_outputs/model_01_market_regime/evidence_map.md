...
| Quality evidence | Coverage, freshness, minimum-history, standardization, and missingness evidence used by `data_quality_score` or acceptance review. |
...
`data_quality_score` summarizes construction coverage. It must not be used as a
risk-on/risk-off market signal by itself.

## Transition pressure

`transition_pressure` is a row-level instability indicator derived from changes
