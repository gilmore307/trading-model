...
from src.review.execute_usdt_calibrate import execute_plan
from src.review.flatten_all import flatten_all_positions
from src.review.prepare_usdt_calibrate import build_calibrate_plan
...
OUT = REPORTS / 'latest-calibrate-orchestrator.json'
...
def post_calibrate_alignment_report(settings: Settings) -> dict:
    registry = OkxClientRegistry(settings)
...
def orchestrate_calibrate(settings: Settings) -> dict:
    ...
    if not flatten.get('ok'):
        note = 'OKX demo calibrate paused: flatten-all failed or positions remain after retry passes. Please close positions manually.'
        notify(settings, note + ' Details: ' + json.dumps(flatten.get('accounts', []), ensure_ascii=False))
        payload = {
            'generated_at': datetime.now(UTC).isoformat(),
            'type': 'calibrate_orchestrator',
    ...
    if not equity.get('all_accounts_ok'):
        notify(
            settings,
            'OKX demo calibrate paused: one or more accounts are below the calibrate equity threshold. '
            'Please reset the demo accounts manually on OKX, then retry calibrate. '
            + json.dumps(equity.get('below_threshold_accounts', []), ensure_ascii=False),
        )
        payload = {
            'generated_at': datetime.now(UTC).isoformat(),
            'type': 'calibrate_orchestrator',
            'status': 'manual_demo_account_reset_required_before_calibrate',
            'flatten': flatten,
            'equity': equity,
        }
        OUT.write_text(json.dumps(payload, indent=2))
    ...
    plan = build_calibrate_plan(settings)
    execution = execute_plan(settings)
    calibrate_bucket_reset = perform_calibrate_bucket_reset(settings)
    alignment = post_calibrate_alignment_report(settings)
    if not alignment.get('ok'):
        payload = {
            'generated_at': datetime.now(UTC).isoformat(),
            'type': 'calibrate_orchestrator',
            'status': 'post_calibrate_alignment_failed',
            'flatten': flatten,
            'equity': equity,
            'plan': plan,
            'execution': execution,
            'calibrate_bucket_reset': calibrate_bucket_reset,
            'post_calibrate_alignment': alignment,
        }
        OUT.write_text(json.dumps(payload, indent=2))
        notify(
            settings,
            'OKX demo calibrate halted after local calibrate bucket reset because exchange/local alignment is still not clean. '
            + json.dumps(alignment, ensure_ascii=False),
        )
        return payload

    set_mode('trade', reason='calibrate_complete_auto_transition', actor='calibrate_orchestrator')
    notify(
        settings,
        'OKX demo calibrate completed automatically. Assets converted to USDT, calibrate bucket reset to '
...
def main() -> None:
    ...
    payload = orchestrate_calibrate(settings)
