...
from src.research.dataset_builder import DEFAULT_HORIZONS, build_research_row, write_jsonl
...
def _parse_horizons(value: str | None) -> dict[str, int]:
    if not value:
    ...
    return result or dict(DEFAULT_HORIZONS)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--horizons', type=str, default=None, help='Optional forward label horizons, e.g. fwd_ret_15m=1,fwd_ret_1h=3')
    return parser
...
def main() -> None:
    ...
    for _ in range(max(1, args.cycles)):
        ...
        rows.append(build_research_row(output=output, horizons=horizons))
