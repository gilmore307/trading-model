...
class TrendExecutor(BaseExecutor):
    ...
    def build_plan(self, output: RegimeRunnerOutput) -> ExecutionPlan:
        ...
        if bg_adx >= 25 and aligned:
            ...
            compressed_pause = p_bw <= 0.015 and trade_burst < 0.4

            if follow_through >= 3.0 and not stretched:
            ...
            if follow_through >= 1.0 and not compressed_pause:
...
class RangeExecutor(BaseExecutor):
    ...
    def build_plan(self, output: RegimeRunnerOutput) -> ExecutionPlan:
        ...
        trend_pressure = bg_adx >= 25 and bg_slope20 * bg_slope50 > 0 and abs(bg_slope20) > 0.5
        bursty = trade_burst >= 0.5 or abs(e_z) >= 1.8

        if p_adx <= 18 and 0.08 <= p_bw <= 0.30:
            reversion_score = 0.0
            if abs(p_z) >= 0.8:
                reversion_score += 1.0
            if abs(e_z) >= 0.5:
...
class CompressionExecutor(BaseExecutor):
    ...
    def build_plan(self, output: RegimeRunnerOutput) -> ExecutionPlan:
        ...
        if bw <= 0.015 and rv <= 0.10:
            ...
            if launch_bias >= 3.0 and not structure_block:
...
class CrowdedExecutor(BaseExecutor):
    ...
    def build_plan(self, output: RegimeRunnerOutput) -> ExecutionPlan:
        ...
        rejection_score = 0.0
        if crowd_extreme >= 0.85:
            rejection_score += 1.0
        if abs(oi_accel) >= 0.15:
            rejection_score += 1.0
        if abs(basis) >= 0.004:
            rejection_score += 1.0
        if abs(p_z) >= 1.2:
        ...
        fast_event_conflict = trade_burst >= 0.8 and abs(e_z) >= 2.5
        if rejection_score >= 4.0 and not fast_event_conflict:
