...
MODE_POLICIES: dict[RuntimeMode, ModePolicy] = {
    ...
    RuntimeMode.REVIEW: ModePolicy(RuntimeMode.REVIEW, allow_strategy_execution=False, force_dry_run=True, allow_normal_routing=False, requires_flatten_workflow=False, description='legacy compatibility mode only: review should run as an external event/job rather than as a daemon runtime mode'),
    RuntimeMode.CALIBRATE: ModePolicy(RuntimeMode.CALIBRATE, allow_strategy_execution=False, force_dry_run=True, allow_normal_routing=False, requires_flatten_workflow=True, description='legacy compatibility mode only: calibrate should be treated as a strategy-upgrade event/helper rather than as a daemon runtime mode'),
