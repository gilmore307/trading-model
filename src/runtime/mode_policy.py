...
MODE_POLICIES: dict[RuntimeMode, ModePolicy] = {
    ...
    RuntimeMode.REVIEW: ModePolicy(RuntimeMode.REVIEW, allow_strategy_execution=False, force_dry_run=True, allow_normal_routing=False, requires_flatten_workflow=False, description='review mode: reporting / review workflow only, then auto-return to calibrate'),
    RuntimeMode.CALIBRATE: ModePolicy(RuntimeMode.CALIBRATE, allow_strategy_execution=False, force_dry_run=True, allow_normal_routing=False, requires_flatten_workflow=True, description='calibrate mode: flatten + convert-to-USDT + bucket reset workflow, then auto-return to trade'),
