...
- Layer 7 is `TradingGuidanceModel / OptionExpressionModel`; option expression uses Layer 7 underlying price-path assumptions plus option-chain evidence for option expression and contract constraints, not live execution.
- Layer 8 is `EventRiskGovernor / EventIntelligenceOverlay`, consuming point-in-time event evidence from `source_04_event_overlay`, event detail artifacts, upstream context refs, and Layer 7 guidance to output `event_risk_intervention` / event-adjusted risk guidance. Local deterministic event-risk scaffold and fixture tests are implemented in `src/models/model_08_event_risk_governor/`.
