...
- Layer 7 trading guidance includes the accepted `OptionExpressionModel` implementation surface for offline `option_expression_plan` / `expression_vector` rows. Its physical package remains `src/models/model_07_option_expression/` until a separate option-expression renumbering migration; it is no longer the conceptual Layer 8 owner.
