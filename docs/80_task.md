...
- Current V2.2 architecture is `MarketRegimeModel -> SectorContextModel -> TargetStateVectorModel -> AlphaConfidenceModel -> PositionProjectionModel -> UnderlyingActionModel -> TradingGuidanceModel / OptionExpressionModel -> EventRiskGovernor / EventIntelligenceOverlay`, with anonymous target candidate construction inside Layer 3 preprocessing.
- Layer 7 trading guidance includes the accepted `OptionExpressionModel` implementation surface for offline `option_expression_plan` / `expression_vector` rows. Its current physical package is `src/models/model_07_option_expression/`; it is no longer the conceptual Layer 8 owner.
...
- Layer 4 is now `AlphaConfidenceModel`, consuming the reviewed Layer 1/2/3 state stack to output `alpha_confidence_vector` with alpha direction, strength, expected residual return, confidence, reliability, path quality, reversal risk, drawdown risk, and alpha tradability. Base/unadjusted Layer 1/2/3 alpha is diagnostic-only. Local deterministic scaffold and fixture tests are implemented in `src/models/model_04_alpha_confidence/`.
- Layer 5 is now `PositionProjectionModel`, consuming final adjusted alpha plus current/pending position, position-level friction, portfolio exposure, and risk-budget context to output `position_projection_vector`; it maps alpha to projected target holding state, not buy/sell/hold operations. Local deterministic scaffold and fixture tests are implemented in `src/models/model_05_position_projection/`.
- Layer 6 is now `UnderlyingActionModel`, consuming Layer 5/6 state plus point-in-time underlying quote/liquidity/current-pending exposure/risk-policy context to output `underlying_action_plan` and `underlying_action_vector`; it maps target exposure to planned direct stock/ETF action thesis, not broker orders. Local deterministic scaffold and fixture tests are implemented in `src/models/model_06_underlying_action/`.
- Layer 7 is `TradingGuidanceModel / OptionExpressionModel`; option expression uses Layer 6 underlying price-path assumptions plus option-chain evidence for option expression and contract constraints, not live execution.
- Layer 8 is `EventRiskGovernor / EventIntelligenceOverlay`, consuming point-in-time event evidence from `source_08_event_risk_governor`, event detail artifacts, upstream context refs, and Layer 7 guidance to output `event_risk_intervention` / event-adjusted risk guidance. Local deterministic event-risk scaffold and fixture tests are implemented in `src/models/model_08_event_risk_governor/`.
...
Event redo closeout artifact added: `scripts/models/model_08_event_risk_governor/build_event_model_closeout_report.py` emits `event_model_closeout_report_v1`. It makes the final judgment actionable: close the redo as bounded Layer 8 EventRiskGovernor / EventIntelligenceOverlay; keep earnings/guidance in direction-neutral scouting; reject broad event alpha, option-flow alpha, and signed guidance claims until PIT expectation/comparison gates exist; preserve diagnostic artifacts; and keep storage deletion on hold until reviewed event regeneration finishes.
