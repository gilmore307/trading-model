...
- Implemented initial generic model governance SQL schema helpers in `src/model_governance/schema.py` and the operational wrapper `scripts/ensure_model_governance_schema.py` for `model_dataset_request`, `model_dataset_snapshot`, `model_dataset_split`, `model_eval_label`, `model_eval_run`, and `model_eval_metric`. The wrapper emits temporary SQL to gitignored `storage/sql/` by default and touches a real database only with explicit `--apply`. Table names are registered in `trading-manager`; concrete column registration remains deferred until the schema has been exercised with the first evaluation harness.
