...
- Event-family impact-window backtest gate: the 2026-06-10 impact-window run is a small contract verifier only, not real-market promotion evidence. The next accepted backtest-contract step is to replace the sample inputs with real CPI release calendars, real triple-witching calendars, real breaking-news samples, and canonical point-in-time price-path inputs before making any promotion-evidence claim.
