...
- Event-family impact-window backtest gate: the 2026-06-10 sample impact-window run remains a contract verifier only. The real-input follow-up run now replaces the sample inputs with local Trading Economics CPI releases, rule-derived triple-witching dates, SQL-retained Alpaca/GDELT breaking-news shock samples, and SQL-retained point-in-time SPY/QQQ/IWM price paths under `/root/projects/trading-storage/storage/03_model_artifacts/event_family_impact_window_real_input_backtest_20260610/`. It is review-ready impact-window evidence, not promotion-approved evidence; promotion use still requires event-family review and the broader Layer 10/Layer 4 promotion gates.
