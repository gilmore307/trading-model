...
- Event-family impact-window backtest gate: the 2026-06-10 sample impact-window run remains a contract verifier only. The real-input follow-up run replaces the sample inputs with local Trading Economics CPI releases, rule-derived triple-witching dates, SQL-retained Alpaca/GDELT breaking-news shock samples, and SQL-retained point-in-time SPY/QQQ/IWM price paths under `/root/projects/trading-storage/storage/03_model_artifacts/event_family_impact_window_real_input_backtest_20260610/`. The first closed-loop replay overlay applies those windows to `fold_2016-01_2016-06` / `model_group_replay_20260609T060059Z` under `/root/projects/trading-storage/storage/03_model_artifacts/event_family_impact_window_replay_20260610/`. These are review-ready Layer 10 evidence artifacts, not promotion-approved evidence; promotion use still requires event-family review and the broader Layer 10/Layer 4 promotion gates.
