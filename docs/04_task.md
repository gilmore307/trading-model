...
- Implemented initial generic model governance SQL schema helpers in `src/model_governance/schema.py` and the operational wrapper `scripts/ensure_model_governance_schema.py` for `model_dataset_request`, `model_dataset_snapshot`, `model_dataset_split`, `model_eval_label`, `model_eval_run`, and `model_eval_metric`. The wrapper has a `--dry-run` SQL preview mode; default execution can create the model governance tables in the development DB. `scripts/clear_model_development_database.py` clears the `trading_model` development schema at the end of a development run with explicit confirmation. Table names are registered in `trading-manager`; concrete column registration remains deferred until the schema has been exercised with the first evaluation harness.
