...
The result is evidence that residual-MLP checkpoints are viable for the residual-MLP-selected layers. It does not by itself promote any layer and does not validate the GRU-selected layers. The active per-layer schemes are fixed by the matrix above and still need their layer-specific labels, replay, and promotion evidence.
