...
Training should not blindly copy live-routing filters when doing so would starve the model of useful contrast. For example, Layer 3 live routing may receive targets from the realtime total-symbol pool with attached Layer 2 context, while Layer 3 historical training may include targets from a broader frozen point-in-time candidate universe so the model learns sector-confirmed, sector-divergent, strong-in-weak-sector, and weak-in-strong-sector behavior.
