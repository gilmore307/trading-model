...
| 5 | `AlphaConfidenceModel` | Broad state candidate rows from many market/sector/target/event-failure contexts and both long/short directional orientations, including negative and low-confidence examples. | Current reviewed Layer 1-4 stack for routed candidates. |
...
| 7 | `UnderlyingActionModel` | Direct underlying/spot action-outcome samples across broad stock, ETF, and crypto-style exposure states, liquidity states, alpha strengths, and cost/risk conditions. Includes maintain/no-trade and adverse examples. | Current position projection for direct-underlying/spot expression candidates. |
| 8 | `EventRiskGovernor` / `EventIntelligenceOverlay` | Cross-target/cross-sector residual-event rows, including event/no-event, effective/ineffective, sector-confirmed/sector-divergent, abnormal activity, news, earnings, macro, and price-action/false-breakout cases, joined point-in-time to the Layer 7 direct-underlying action thesis. | Residual events attached to the current routed target/context stack and underlying/spot thesis for warning/intervention only; option-expression context is not required. |
| 9 | `TradingGuidanceModel` / `OptionExpressionModel` | Final offline trading-guidance rows and optionable-underlying samples with historical option-chain snapshots across IV regimes, expiries, deltas, liquidity, trends, chops, event-risk interventions, and no-expression cases. | Current Layer 7 underlying thesis plus Layer 8 event-risk context and current option-chain context when options are available; direct-underlying/crypto routes may produce no-option guidance. |
