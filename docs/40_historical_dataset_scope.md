...
| 9 | `EventRiskGovernor` / `EventIntelligenceOverlay` | Cross-target/cross-sector residual-event rows, including event/no-event, effective/ineffective, sector-confirmed/sector-divergent, abnormal activity, news, earnings, macro, and price-action/false-breakout cases, joined point-in-time to the Layer 7 direct-underlying action thesis plus optional Layer 8 expression context. | Residual events attached to the current routed target/context stack and underlying/spot thesis for warning/intervention only; option-expression context is optional and absent for crypto/direct-underlying-only routes. |
