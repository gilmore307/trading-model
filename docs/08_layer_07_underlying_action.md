...
Layer 5 asks whether adjusted alpha exists. Layer 6 maps adjusted alpha to target holding state. Layer 7 maps target holding state to a direct underlying action thesis. Layer 8 may use that thesis to choose an option expression. `trading-execution` is the only owner of real order placement.
...
Layer id: model_07_underlying_action
...
current state + confidence + target exposure
  ...
  -> Layer 8 underlying-path handoff
```
...
Layer 7 may consume Layer 5 score families directly or through Layer 6 refs when it needs the alpha/path/risk terms behind an action plan:
...
`underlying_event_policy_block_state` is policy-derived. Layer 4 event context does not directly issue hard trade blocks; event risk may be translated into policy state by Layer 5/6/policy logic before Layer 7 consumes it.

## V1 horizons

Layer 7 V1 uses the synchronized horizons already used by Layers 4, 5, and 6:
...
3. Layer 6 gap-only threshold baseline;
...
8. full Layer 7 with Layer 8 handoff fields.
