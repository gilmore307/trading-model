...
- `05_layer_04_event_failure_risk.md` — Layer 4 EventFailureRiskModel contract, reviewed event/strategy-failure conditioning, `event_failure_risk_vector`, promotion gate, and boundaries.
- `06_layer_05_alpha_confidence.md` — Layer 5 AlphaConfidenceModel contract, base-alpha diagnostics policy, `alpha_confidence_vector`, labels, baselines, and boundaries.
...
- `09_layer_08_trading_guidance.md` — Layer 8 TradingGuidanceModel / OptionExpressionModel contract, base trading-guidance boundary, optional option-expression plan/vector, labels, baselines, and invariants.
- `10_layer_09_event_risk_governor.md` — Layer 9 EventRiskGovernor / EventIntelligenceOverlay contract, event-risk intervention status, evidence requirements, and broker-mutation boundary.
...
- `93_state_vector_feature_registry.md` — accepted semantic guardrails for Layer 1-9 score direction, quality, risk, scope, liquidity, routing, diagnostics, and research-only fields.
- `94_model_stack_closeout.md` — model-design closeout for Layers 1-9 after the EventFailureRiskModel insertion.
- `95_promotion_readiness.md` — production-promotion readiness matrix and evidence checklist for Layers 1-9; no production approval implied.
- `96_promotion_closeout.md` — current production-promotion closeout evidence receipt: real-evidence deferrals for Layers 1-3 and explicit production-evidence blockers for Layers 4-9; durable decisions/activation live in `trading-manager`.
...
- [100_event_family_scouting.md](100_event_family_scouting.md) — Layer 9 event-family scouting contract, fine-grained family batch catalog/closeout, all-family precondition packets, local empirical coverage scan, status values, early-stop rules, current option/news/CPI findings, and the promotion path into Layer 4 EventFailureRiskModel.
...
- [102_event_layer_final_judgment.md](102_event_layer_final_judgment.md) — accepted final current-cycle event-layer judgment: build EventRiskGovernor as bounded risk/intelligence overlay, accept only CPI and earnings shells for risk/control, and do not promote broad event alpha or standalone option abnormality.
- [103_event_governor_layer_04_mechanism.md](103_event_governor_layer_04_mechanism.md) — detailed EventGovernor/Layer 8 base-guidance to Layer 4 promotion mechanism, including abnormality categories, lifecycle types, lifecycle states, clocks, and bridge relation timing.
