...
Rationale: the raw Alpaca-news family slice labeled `earnings_or_guidance_news` was the strongest diagnostic slice in `/root/projects/trading-storage/storage/model/option_event_risk_amplifier_study_20260515/`, but it had only 20 windows across 4 symbols and used headline keywords rather than canonical earnings/report artifacts. That supports scouting only.
...
```text
trading-storage/storage/manager/earnings_guidance_baseline/nasdaq_earnings_calendar/YYYY-MM-DD/task_key.json
```
...
Artifact: `/root/projects/trading-storage/storage/model/earnings_guidance_prior_official_baseline_source_audit_q4_2025_20260515/`
...
Artifact: `/root/projects/trading-storage/storage/model/earnings_guidance_prior_official_document_coverage_q4_2025_20260515/`
...
After the first prior-filing selector produced low coverage, a refined pass targeted official prior-quarter earnings/outlook-bearing SEC exhibits rather than primary 8-K wrapper text. The fetched exhibit set lives under `/root/projects/trading-storage/storage/data/earnings_guidance_prior_earnings_window_exhibit_text_q4_2025_20260515/` and the extraction study itself performs zero provider calls.
