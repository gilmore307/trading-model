...
Historical training may sample a broader target universe than live routing. In live routing, Layer 3 candidates come from the reviewed realtime candidate universe, target metadata, current market-wide hot/liquid names, and point-in-time liquidity, spread, data quality, and optional optionability diagnostics. Current historical replay uses the fixed `historical_candidate_universe.csv` table seeded from the current realtime pool plus BTC, ETH, and SOL, not the mutable realtime pool itself. Equity rows preserve TradingView sector classification and map to accepted SPDR Layer 2 anchors; crypto rows map to BKCH. Layer 2 context remains attached as point-in-time context; it is not an unconditional historical-training filter and does not define the candidate universe.
...
Layer 3 may use ThetaData option-chain evidence starting at the target-state stage, but only after reducing a bounded source envelope into role-selected target-level market-context states. This is a chain-state reduction contract, not an option-expression or executable contract-selection route. Source acquisition defaults to `max_dte=180` and a narrow strike range; the deterministic reducer, not the provider request, owns the model-facing role selector.

Canonical stable-core reduction:
...
- `oi_activity_attention`: point-in-time OI attention candidate when OI is observable.
- `short_expiry_pressure_overlay`: `0-6 DTE`, reported separately from stable structural core.
...
- `target_option_flow_pressure_state`: call/put activity pressure from point-in-time activity attention roles and observable OI where available, normalized to target baseline and worded as activity/pressure, not alpha. Same-snapshot volume, trade count, and notional may support state measurement or validation but must not be used as prefetch selector inputs.
...
- Layer 5 AlphaConfidenceModel and later consumers, not Layer 3, own direction-confidence calibration, target/stop/action projection, position sizing, expression, and final trading guidance. Layer 10 EventRiskGovernor owns post-guidance event-risk intervention.
