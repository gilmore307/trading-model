...
Build the event layer now only as a bounded **M03 Event State / M03 Event State Overlay**.
...
+ abnormal-activity bridge evidence as provenance/risk context only after a non-overlap check against upstream model inputs
  -> M03 Event State / M03 Event State Overlay
  -> event risk control, uncertainty, review/block/cap/flatten hints
```
...
- `/root/projects/trading-storage/storage/03_model_artifacts/earnings_guidance_event_scouting_20260515/`
...
3. **Event/activity bridge** — typed relationships such as pre-event precursor, co-event reaction, post-event absorption, divergence, and unresolved latent hazard, admitted for scoring only when the activity leg is not already counted by `base_stack_layers_01_09` or remains residual after upstream conditioning.
4. **M03 event-effect output** — uncertainty, gap/reversal/liquidity/contagion risk, distribution-shape changes, gate/exposure pressure, and audit explanations.
...
|---|---:|---|
| `risk_control_feature_accepted_not_alpha` | 1 | CPI actual-vs-expectation surprise may be used as macro event-risk-shape context after canonical TE history is complete. |
...
Accepted now for risk-shape only:
...
Allowed M03 Event State outputs:
...
- component-control pressure evidence;
- audit explanation and source refs.

Prohibited M03 Event State outputs:
...
1. Keep the M03 Event State structure and registry contracts.
2. Regenerate the M03 Event State using the allowed risk/intelligence output contract only.
3. Add CPI surprise as macro event-risk-shape context after fuller TE expectation-history coverage is available.
...
Judgment update: the earnings+option amplifier comparison remains blocked, not positive or negative. Continuing this route requires either broader-chain no-abnormality verification that actually yields clean controls, or a different control design; the M03 Event State boundary does not expand.
...
Judgment update: SEC metadata and normalized facts are not enough. Earnings/guidance remains direction-neutral M03 Event State context only until official company document text is acquired, interpreted under a reviewed guidance standard, and joined to point-in-time expectation baselines.
...
Judgment update: this resolves the previous local-document blocker, but not the signed-claim blocker. All 12 documents remain uninterpreted for guidance/outlook, and there are still zero expectation baselines and zero signed-direction-ready rows. Earnings/guidance remains direction-neutral M03 Event State context only.
...
Judgment update: this narrows the human/reviewed interpretation queue, but it is not accepted guidance interpretation. SEC filings contain generic expectation/accounting/safe-harbor language, so candidate text remains review-required. Signed-direction and stronger M03 Event State claims remain blocked by missing reviewed guidance/result interpretation and missing point-in-time expectation baselines.
...
Judgment update: this is useful event context, but not signed alpha. Accepted guidance raise/cut remains zero, expectation baselines remain zero, and signed-direction-ready rows remain zero. Earnings/guidance can support direction-neutral M03 Event State context only.
...
- `/root/projects/trading-storage/storage/03_model_artifacts/earnings_guidance_prior_official_baseline_source_audit_q4_2025_20260515/`
- `/root/projects/trading-storage/storage/03_model_artifacts/earnings_guidance_prior_official_document_coverage_q4_2025_20260515/`
