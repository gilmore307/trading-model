...
Layer 3 builds a point-in-time, direction-neutral `target_context_state` that later layers may use for event context, alpha confidence, position projection, and expression/final-action handoff. Layer 3 does not output position size or final action.
