...
| 4 | `EventRiskGovernor` | Cross-target/cross-sector event rows, including event/no-event, effective/ineffective, sector-confirmed/sector-divergent, abnormal activity, news, earnings, macro, and price-action/false-breakout cases. | Events attached to the current routed target/context stack. |
| 5 | `AlphaConfidenceModel` | Broad state+event candidate rows from many market/sector/target contexts and both long/short directional orientations, including negative and low-confidence examples. | Current reviewed Layer 1-4 stack for routed candidates. |
...
| 8 | `OptionExpressionModel` | Optionable-underlying samples with historical option-chain snapshots across IV regimes, expiries, deltas, liquidity, event windows, trends, chops, and no-expression cases. | Current Layer 7 underlying thesis plus current option-chain context for optionable candidates only. |
