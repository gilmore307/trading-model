...
| 4 | `EventOverlayModel` | Cross-target/cross-sector event rows, including event/no-event, effective/ineffective, sector-confirmed/sector-divergent, abnormal activity, news, earnings, macro, and price-action/false-breakout cases. | Events attached to the current routed target/context stack. |
