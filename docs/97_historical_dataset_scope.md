...
| 4 | `EventFailureRiskModel` | Reviewed event/strategy-failure families across market/sector/target contexts, including accepted risk/control cases, rejected controls, and non-applicable contexts. | Only event/strategy-failure factors accepted by evidence packet plus agent/manager review. |
| 5 | `AlphaConfidenceModel` | Broad state candidate rows from many market/sector/target contexts and both long/short directional orientations, including negative and low-confidence examples, with Layer 4 conditioning when accepted. | Current reviewed Layer 1-3 stack plus accepted Layer 4 event-failure-risk conditioning when applicable. |
| 6 | `PositionProjectionModel` | Candidate alpha rows paired with simulated or historical current/pending position states, costs, risk budgets, and exposure constraints. Scenarios may be constructed offline if point-in-time and label-safe. | Current alpha plus actual/approved current and pending position context. |
| 7 | `UnderlyingActionModel` | Direct stock/ETF action-outcome samples across broad exposure states, liquidity states, alpha strengths, and cost/risk conditions. Includes maintain/no-trade and adverse examples. | Current position projection for direct-underlying expression candidates. |
| 8 | `TradingGuidanceModel` / `OptionExpressionModel` | Base trading-guidance rows and optionable-underlying samples with historical option-chain snapshots across IV regimes, expiries, deltas, liquidity, trends, chops, and no-expression cases. | Current Layer 7 underlying thesis plus current option-chain context for optionable candidates only. |
| 9 | `EventRiskGovernor` / `EventIntelligenceOverlay` | Cross-target/cross-sector event rows, including event/no-event, effective/ineffective, sector-confirmed/sector-divergent, abnormal activity, news, earnings, macro, price-action/false-breakout cases, and residual anomalies joined point-in-time to base guidance. | Reviewed realtime observation-pool families plus current routed target/context stack and Layer 8 base guidance. |
