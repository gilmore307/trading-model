...
Status: active architecture revision. Layer 9; current V1 physical implementation surface is `src/models/model_09_option_expression/` as the option-expression subset until a dedicated broader TradingGuidanceModel implementation is accepted.

Active boundary: Layer 9 outputs an offline `trading_guidance_record` and optional option-expression context. It consumes the Layer 7 underlying-action thesis, Layer 8 event-risk context/intervention evidence, position/risk context, and optional point-in-time option-chain evidence. It may choose direct-underlying, option-expression, or no-trade/maintain guidance as an offline decision record.
...
Layer 9 is downstream of the Layer 8 governor. Crypto/direct-underlying-only routes may bypass option-expression planning inside Layer 9, but still use the Layer 7 thesis plus Layer 8 event-risk context for final offline guidance.
<!-- /ACTIVE_LAYER_REVISION -->
