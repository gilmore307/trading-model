...
`trading-data` owns the deterministic Layer 3 strategy selection feature production, including per-bar variant simulation. `trading-model` owns oracle construction, variant expansion/pruning proposals, StrategySelectionModel training, promotion evidence, and agent-reviewed lifecycle decisions. The review script may request `feature_03_strategy_selection` through `trading-manager`, then consume the completed feature surface; it should not implement the data-production runner itself.
