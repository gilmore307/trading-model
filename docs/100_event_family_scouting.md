...
Coverage result: 5 families have existing empirical/scouting artifacts; 12 families have local candidate events but still need interpretation/dedup/matched-control association work; 7 families have no local candidate events under current source/parser coverage; 3 are blocked on PIT expectation/comparable baselines; 3 are blocked on residual detectors; 1 is blocked on liquidity/depth evidence; option derivatives abnormality remains deferred until its abnormality definition is revised. The final association conclusion remains withheld.
