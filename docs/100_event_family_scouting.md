...
If a family proves stable, predictive, and incremental across controls, splits, base-stack residuals, and regimes, it may be proposed for strategy-layer promotion. Strategy-layer promotion requires a script-emitted evidence packet and final agent review. Until that reviewed promotion exists, the event family remains a correction/explanation/warning overlay, not a strategy decision input.
