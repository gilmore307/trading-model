...
The first closed-loop replay overlay is generated by:

```bash
PYTHONPATH=src python3 scripts/models/model_10_event_risk_governor/build_event_family_impact_window_replay.py
```

Output artifacts live under `/root/projects/trading-storage/storage/03_model_artifacts/event_family_impact_window_replay_20260610/fold_2016-01_2016-06/model_group_replay_20260609T060059Z/`:
...
This artifact completes the fold1 audit by assigning all 31 visible/active families explicit packet, canonical parser/source, matched-control, impact-window, fold-stability, and leakage/upstream-overlap dispositions. Fold1 evidence is complete because every family now has a gate disposition. Production evidence is not complete: 3 families are calibrated-overlay/progress rows (`cpi_inflation_release`, `triple_witching_calendar`, `breaking_news_shock`), 28 remain diagnostic keyword-overlay rows, and 0 families are production-complete. Safety flags remain zero/false for provider calls, SQL writes, model training, model activation, broker/account mutation, and artifact deletion.
