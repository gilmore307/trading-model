...
Status: active M03 event-governance tooling M03 event-state governance research/governance contract.
...
Each event family must have a packet before model training or risk-control promotion work begins.
...
```bash
PYTHONPATH=src python3 scripts/models/model_03_event_state/event_governance/build_event_family_batch_catalog.py
```
...
```bash
PYTHONPATH=src python3 scripts/models/model_03_event_state/event_governance/build_event_price_association_readiness.py
```
...
```bash
PYTHONPATH=src python3 scripts/models/model_03_event_state/event_governance/build_cpi_inflation_association_readiness.py
```
...
The TE-only sample is too small for a final claim, but it is directionally consistent with the surprise framing: use `actual - consensus` when present, otherwise `actual - te_forecast`, and treat large CPI surprise as macro event-risk-shape input rather than standalone alpha.
...
```bash
PYTHONPATH=src python3 scripts/models/model_03_event_state/event_governance/build_event_family_remaining_acceptance.py
```
...
- 2 risk-shape candidates only: `earnings_guidance_scheduled_shell` and `cpi_inflation_release`;
...
```bash
PYTHONPATH=src python3 scripts/models/model_03_event_state/event_governance/build_event_family_precondition_completion.py
```
...
```bash
PYTHONPATH=src python3 scripts/models/model_03_event_state/event_governance/build_event_family_empirical_coverage.py
```
...
```bash
PYTHONPATH=src python3 scripts/models/model_03_event_state/event_governance/build_event_layer_final_judgment.py
```
...
Final answer: build the event layer as `M03 Event State / M03 Event State Overlay`, not as a standalone event-alpha model. Current evidence accepts `cpi_inflation_release` and `earnings_guidance_scheduled_shell` for risk-shape only. No family is accepted for standalone directional alpha. All other families remain blocked, deferred, or research-queue only until their family-specific evidence gates are satisfied.
...
```bash
PYTHONPATH=src python3 scripts/models/model_03_event_state/event_governance/build_event_family_all_association.py
```
...
```bash
PYTHONPATH=src python3 scripts/models/model_03_event_state/event_governance/build_event_family_impact_window_real_input_backtest.py
```
...
```bash
PYTHONPATH=src /root/projects/trading-manager/.venv/bin/python scripts/models/model_03_event_state/event_governance/build_event_family_impact_window_all_family_real_input_backtest.py
```
...
The dated closed-loop replay overlay and fold-completion matrix remain historical contract evidence. They are not the current fold-cadence route and should not be used as operator run instructions. Current event impact analysis is projected through the replay-review read models and the Events/Temporary Exploring dashboard surface, where event scope can be compared against replay performance, decision, and operation evidence.
...
```bash
PYTHONPATH=src python3 scripts/models/model_03_event_state/event_governance/build_event_family_threshold_grading.py
```
...
Active threshold universe now contains 11 families: accepted risk-shape seeds `earnings_guidance_scheduled_shell` and `cpi_inflation_release`, plus nine expanded-screening threshold-review candidates (`earnings_call_narrative_residual`, `equity_offering_dilution`, `buyback_or_capital_return`, `customer_contract_win_loss`, `management_change`, `analyst_rating_or_price_target_change`, `supply_chain_disruption`, `sector_regulation_policy`, and `commodity_or_input_cost_shock`).
...
```bash
PYTHONPATH=src python3 scripts/models/model_03_event_state/event_governance/build_price_anomaly_event_discovery.py
```
...
`base_stack_models_01_05`
  ...
  -> M03 event-governance tooling event-family explanation / effect-model evidence
```

This makes reverse discovery stricter. A price move should only become an event-model candidate when it is abnormal after the base stack explains background context, target state, event state, unified decision, and option/trading guidance. The M03 event-governance tooling then answers whether nearby events share a common family, whether the event plausibly explains the residual, and whether the right output is uncertainty increase, gate pressure, exposure pressure, tail-shape evidence, or no event adjustment.

This route supports the final target: M03 event-governance tooling M03 event-state governance remains a coverage, correction, explanation, and warning model. It is not a replacement for the base trading model and does not become a standalone event-alpha model.
...
```bash
PYTHONPATH=src python3 scripts/models/model_03_event_state/event_governance/build_event_observation_pool_policy.py
```
...
If a family proves stable, predictive, and incremental across controls, splits, base-stack residuals, and regimes, it may be proposed for M03 EventStateModel promotion. Promotion requires a script-emitted evidence packet and final agent review using the fixed `event-strategy-promotion-review` skill. Until that reviewed promotion exists, the event family remains an M03 event-governance tooling effect-model evidence packet, not a front decision input.

The residual-anomaly discovery implementation is generated by:

```bash
PYTHONPATH=src python3 scripts/models/model_03_event_state/event_governance/build_residual_anomaly_event_discovery.py
```
