...
This run uses the event-family packets plus SQL-retained PIT candidate events, Trading Economics CPI rows, deterministic triple-witching dates, and point-in-time SPY/QQQ/IWM bars. It emits 31 calibrated fold-visible/active family windows. The evidence is fold-calibration complete and still requires cross-fold stability and production-route review before promotion.
...
This artifact completes the fold1 audit by assigning all 31 visible/active families explicit packet, canonical parser/source, matched-control, impact-window, fold-stability, and leakage/upstream-overlap dispositions. Fold1 evidence is complete because every family now has a calibrated fold window and a gate disposition. Production evidence is not complete: all 31 families remain production-pending because cross-fold stability and production-route review are still required. Safety flags remain zero/false for provider calls, SQL writes, model training, model activation, broker/account mutation, and artifact deletion.
