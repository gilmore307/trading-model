...
Status: active M06 ResidualEventGovernance research/governance contract.
...
Artifact: `/root/projects/trading-storage/storage/03_model_artifacts/option_activity_matched_control_study_20260515/`
...
Artifact: `/root/projects/trading-storage/storage/03_model_artifacts/option_activity_strict_filter_study_20260515/`
...
Artifact: `/root/projects/trading-storage/storage/03_model_artifacts/option_event_risk_amplifier_study_20260515/`
...
The dedicated packet is `docs/52_earnings_guidance_event_family_packet.md`.
...
```bash
PYTHONPATH=src python3 scripts/models/model_06_residual_event_governance/build_event_family_batch_catalog.py
```
...
```bash
PYTHONPATH=src python3 scripts/models/model_06_residual_event_governance/build_event_price_association_readiness.py
```
...
```bash
PYTHONPATH=src python3 scripts/models/model_06_residual_event_governance/build_cpi_inflation_association_readiness.py
```
...
Trading Economics storage rows expose `actual`, `consensus`, and `te_forecast` fields through the canonical storage snapshot. A TE-only probe over monthly captured calendar artifacts found expectation-populated historical CPI rows for 18 release dates / 36 CPI metric rows, mostly 2016-2017 YoY rows; later captured rows often retain `actual`/`previous` but blank `consensus`/`te_forecast`. TE storage therefore remains the preferred canonical route when expectation fields are populated, but the available captured sample is not enough by itself for the broad 2017-2026 surprise study. The larger Investing.com surprise diagnostic remains temporary evidence until a fuller accepted expectation-history route exists.
...
## Remaining batch acceptance

The full remaining queue acceptance is generated by:

```bash
PYTHONPATH=src python3 scripts/models/model_06_residual_event_governance/build_event_family_remaining_acceptance.py
```

Output artifacts live under `storage/event_family_remaining_acceptance_20260516/`:

- `event_family_remaining_acceptance.json`
- `event_family_remaining_acceptance_summary.json`
- `event_family_remaining_acceptance.csv`
- `event_family_next_packet_queue.csv`

The acceptance accounts for all 29 fine-grained families without provider calls, model activation, broker/account mutation, or artifact deletion. It promotes no family to standalone directional alpha. Current dispositions are:
...
After the remaining-batch acceptance, the next safe step is to fill the precondition surface for every family before making any final association conclusion. This is generated by:

```bash
PYTHONPATH=src python3 scripts/models/model_06_residual_event_governance/build_event_family_precondition_completion.py
```
...
```bash
PYTHONPATH=src python3 scripts/models/model_06_residual_event_governance/build_event_family_empirical_coverage.py
```
...
```bash
PYTHONPATH=src python3 scripts/models/model_06_residual_event_governance/build_event_layer_final_judgment.py
```
...
```bash
PYTHONPATH=src python3 scripts/models/model_06_residual_event_governance/build_event_family_all_association.py
```
...
## Impact-window backtest

M06 event-governance evidence is not complete merely because one closed-loop replay overlay exists. A family is not validated until its normal event-family workflow has passed: event-family packet, canonical parser/source routing, matched controls, impact-window backtest, fold stability, and leakage/upstream-overlap review. Rows that do not pass that workflow must stay `keyword_sql_observation_day_unvalidated` or another explicit diagnostic status, not `calibrated_impact_window`.

The event-family impact-window sample backtest is generated by:

```bash
PYTHONPATH=src python3 scripts/models/model_06_residual_event_governance/build_event_family_impact_window_backtest.py
```

Output artifacts live under `storage/event_family_impact_window_backtest_20260610/`:

- `event_family_impact_window_backtest.json`
- `event_family_impact_window_backtest_summary.json`
- `event_family_impact_window_backtest.csv`
- `event_family_impact_window_candidate_scores.csv`

This is a contract verifier for M06 event-family parameterization. It enumerates candidate windows and selects the highest event-vs-control risk-delta window for three sample temporal forms: CPI-style scheduled data releases, triple-witching-style scheduled calendar events, and breaking-news-style unscheduled shocks. The sample run selected `minus_7_to_plus_3` for CPI, `minus_2_to_plus_2` for triple witching, and `event_day_only` for breaking news. These are sample-path selections, not accepted real-market promotion evidence.

The reusable runner also supports reviewed real-input contract runs:

```bash
PYTHONPATH=src python3 scripts/models/model_06_residual_event_governance/build_event_family_impact_window_backtest.py \
  --event-csv <reviewed_event_instances.csv> \
  --bar-csv <point_in_time_price_bars.csv> \
  --output-dir <artifact_dir>
```

`--event-csv` and `--bar-csv` are repeatable and must be provided together. Event CSV inputs must include `family_key`, `event_temporal_form`, and an event date field such as `event_date`, `scheduled_date`, `published_date`, `event_time`, or `published_time`; `event_ref` and `source_ref` are preserved when present. Price CSV inputs must include `symbol` and a date/time field plus OHLC fields (`open`/`high`/`low`/`close` or `bar_open`/`bar_high`/`bar_low`/`bar_close`). Intraday bars are aggregated to daily OHLC before scoring. Real-input runs record `input_scope=real_input_backtest` and source paths in the artifact, but still require review before promotion-evidence use.

The 2026-06-10 real-input follow-up is generated by:

```bash
PYTHONPATH=src python3 scripts/models/model_06_residual_event_governance/build_event_family_impact_window_real_input_backtest.py
```

Output artifacts live under `/root/projects/trading-storage/storage/03_model_artifacts/event_family_impact_window_real_input_backtest_20260610/`:

- `input_summary.json`
- `inputs/reviewed_event_instances.csv`
- `inputs/point_in_time_price_bars.csv`
- `backtest/event_family_impact_window_backtest.json`
- `backtest/event_family_impact_window_backtest_summary.json`
- `backtest/event_family_impact_window_backtest.csv`
- `backtest/event_family_impact_window_candidate_scores.csv`

Input coverage is 2016-01-01 through 2026-03-01 exclusive for SPY, QQQ, and IWM. It uses local Trading Economics CPI calendar rows, deterministic quarterly third-Friday triple-witching dates, SQL-retained Alpaca/GDELT breaking-news shock samples, and SQL-retained point-in-time bars. The run produced 115 `cpi_inflation_release` events, 40 `triple_witching_calendar` events, 24 `breaking_news_shock` dates, and 7,662 daily price rows. It made zero provider calls, SQL writes, model-training calls, model activation changes, broker/account mutations, or artifact deletions.

Selected real-input windows:

| Event family | Temporal form | Selected window | Selection score |
|---|---|---:|---:|
| `cpi_inflation_release` | `scheduled_data_release_event` | `minus_3_to_event` | 0.010053 |
| `triple_witching_calendar` | `scheduled_calendar_event` | `minus_3_to_event` | 0.006095 |
| `breaking_news_shock` | `instantaneous_unscheduled_event` | `minus_2_to_plus_2` | 0.090183 |

This closes the impact-window contract through real local inputs, but it is still not promotion-approved evidence. The artifact is `requires_review_before_promotion_evidence_use`; any M03 or production use still needs event-family review, matched-control/stability/leakage review, incremental value evidence, and the accepted promotion route.

The all-family real-input impact-window follow-up is generated by:

```bash
PYTHONPATH=src /root/projects/trading-manager/.venv/bin/python scripts/models/model_06_residual_event_governance/build_event_family_impact_window_all_family_real_input_backtest.py
```

Output artifacts live under `/root/projects/trading-storage/storage/03_model_artifacts/event_family_impact_window_all_family_real_input_backtest_20260610/`:
...
This run uses the event-family packets plus SQL-retained PIT candidate events, Trading Economics CPI rows, deterministic triple-witching dates, and point-in-time SPY/QQQ/IWM bars. It emits 31 calibrated fold-visible/active family windows. The evidence is fold-calibration complete and ready for per-family production-route review. Cross-fold stability is not a prerequisite for focus-pool admission because a family must enter the focus pool before later folds can collect comparable stability evidence.

The all-family closed-loop replay overlay is generated by:

```bash
PYTHONPATH=src /root/projects/trading-manager/.venv/bin/python scripts/models/model_06_residual_event_governance/build_event_family_impact_window_replay.py \
  --event-csv /root/projects/trading-storage/storage/03_model_artifacts/event_family_impact_window_all_family_real_input_backtest_20260610/inputs/reviewed_event_instances.csv \
  --impact-window-summary /root/projects/trading-storage/storage/03_model_artifacts/event_family_impact_window_all_family_real_input_backtest_20260610/backtest/event_family_impact_window_backtest_summary.json \
  --output-dir /root/projects/trading-storage/storage/03_model_artifacts/event_family_impact_window_all_family_replay_20260610/fold_2016-01_2016-06/model_group_replay_20260609T060059Z \
  --no-sql-candidate-events
```

Output artifacts live under `/root/projects/trading-storage/storage/03_model_artifacts/event_family_impact_window_all_family_replay_20260610/fold_2016-01_2016-06/model_group_replay_20260609T060059Z/`:

- `event_family_impact_window_replay_summary.json`
- `m06_residual_event_governance_replay_input_rows.jsonl`
- `model_06_residual_event_governance_rows.jsonl`
- `decision_event_overlay_rows.jsonl`

This run applies the selected all-family real-input windows to the frozen `promotion_replay_candidate_policy` replay decision rows for `fold_2016-01_2016-06`. It generated 1,254 retained migration-source event-governance rows from replay decisions, with 1,122 decisions carrying at least one point-in-time visible event overlay across 31 event families.

Window policy counts are 5,315 `calibrated_impact_window` event matches and zero `keyword_sql_observation_day_unvalidated` matches. Replay overlay result: event-overlay decisions had mean excess return about -0.000599 versus about -0.000636 for non-event-overlay decisions. This is fold-scoped M06 evidence only. It is not a standalone directional-alpha claim, not a production promotion decision, and not an M03 acceptance packet.

The fold-scoped M06 completion matrix is generated by the retained migration-source runner:

```bash
PYTHONPATH=src /root/projects/trading-manager/.venv/bin/python scripts/models/model_06_residual_event_governance/build_m06_residual_event_governance_fold_completion.py
```

Output artifacts live under `/root/projects/trading-storage/storage/03_model_artifacts/m06_residual_event_governance_fold_completion_20260610/fold_2016-01_2016-06/model_group_replay_20260609T060059Z/`:

- `m06_residual_event_governance_fold_completion_summary.json`
- `m06_residual_event_governance_fold_completion.json`
- `m06_residual_event_governance_family_gate_matrix.csv`

This artifact completes the fold1 audit by assigning all 31 visible/active families explicit packet, canonical parser/source, matched-control, impact-window, fold-stability, leakage/upstream-overlap, and production-route review dispositions. Fold1 evidence is complete because every family now has a calibrated fold window and a workflow-level review decision. Production-route review is per family, not an all-family pass/fail gate: 30 families enter the temporal-attention focus pool, while `option_derivatives_abnormality` is rejected under the current definition because its matched-control standard already failed and it needs a revised abnormality definition. Cross-fold stability is follow-up monitoring for focus-pool families, not a prerequisite that blocks admission and prevents later-fold evidence collection. Safety flags remain zero/false for provider calls, SQL writes, model training, model activation, broker/account mutation, and artifact deletion.
...
```bash
PYTHONPATH=src python3 scripts/models/model_06_residual_event_governance/build_event_family_threshold_grading.py
```
...
```bash
PYTHONPATH=src python3 scripts/models/model_06_residual_event_governance/build_price_anomaly_event_discovery.py
```
...
```text
`base_stack_models_01_05`
  -> expected background/target/event/decision/option context
  -> residual_anomaly_context when observed price/path/volume/liquidity/option behavior is not explained
  -> M06 event-family explanation / warning / correction overlay
```

This makes reverse discovery stricter. A price move should only become an event-model candidate when it is abnormal after the base stack explains background context, target state, event state, unified decision, and option/trading guidance. The event-governance model then answers whether nearby events share a common family, whether the event plausibly explains the residual, and whether the right output is warning, block/cap, reduce/flatten review, uncertainty increase, or no event adjustment.

This route supports the final target: M06 ResidualEventGovernance remains a coverage, correction, explanation, and warning model. It is not a replacement for the base trading model and does not become a standalone event-alpha model.
...
```bash
PYTHONPATH=src python3 scripts/models/model_06_residual_event_governance/build_event_observation_pool_policy.py
```
...
If a family proves stable, predictive, and incremental across controls, splits, base-stack residuals, and regimes, it may be proposed for M03 EventStateModel promotion. Promotion requires a script-emitted evidence packet and final agent review using the fixed `event-strategy-promotion-review` skill. Until that reviewed promotion exists, the event family remains an M06 correction/explanation/warning overlay, not a front decision input.

The residual-anomaly discovery implementation is generated by:

```bash
PYTHONPATH=src python3 scripts/models/model_06_residual_event_governance/build_residual_anomaly_event_discovery.py
```
