...
The next backtest-contract step must keep the same output contract but replace the synthetic/sample event inputs with real CPI release calendars, real triple-witching calendars, real breaking-news samples, and canonical point-in-time price-path inputs. Until that real-data route is run and reviewed, the impact-window artifact remains contract-validation evidence only.

The same runner now supports real-input contract runs:
