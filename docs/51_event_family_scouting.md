...
Status: active Layer 10 EventRiskGovernor research/governance contract.
...
This closes the impact-window contract through real local inputs, but it is still not promotion-approved evidence. The artifact is `requires_review_before_promotion_evidence_use`; any Layer 4 or production use still needs event-family review, matched-control/stability/leakage review, incremental value evidence, and the accepted promotion route.
...
`base_stack_layers_01_09`
  ...
  -> Layer 10 event-family explanation / warning / correction overlay
```
